...
#ifndef ESTTS_GROUNDSTATION_CMDTELEM_MANAGER_H
#define ESTTS_GROUNDSTATION_CMDTELEM_MANAGER_H
...
class groundstation_cmdtelem_manager {
private:
...
public:
    /**
     * @brief Default constructor for ground station command/telemetry manager. Takes argument for a transmission
     * interface and initializes the command handler. Note that in order to run the dispatcher, dispatcher_init must be called.
     */
    groundstation_cmdtelem_manager(transmission_interface * ti, std::function<estts::Status(std::string)> telem_callback);
...
#endif //ESTTS_GROUNDSTATION_CMDTELEM_MANAGER_H
